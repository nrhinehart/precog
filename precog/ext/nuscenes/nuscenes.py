...
class NuScenes:
    ...
    def __make_reverse_index__(self, verbose: bool) -> None:
        ...
        # Dictionary mapping scene names to their linear indices.
    ...
    def get_scene_samples(self, scene_name):
        scene_idx = self.scene_name_to_scene_idx[scene_name]
        scene = self.scene[scene_idx]
        first_sample = self.get('sample', scene['first_sample_token'])
        return traverse_linked_list(self, first_sample, tablekey='sample', direction='next', inclusive=True)
    ...
    def get_sample_data(self, sample_data_token: str,
        ...
        for box in boxes:
            ...
            # Determine if the box meets visibility criteria
            vis_indicators.append(box_in_image(box, cam_intrinsic, imsize, vis_level=box_vis_level))

            # If we're returing indicators, we should return every box
            if return_vis_indicators:
                box_list.append(box)
            # If we're not returning indicators, don't add the box if it's not visible on the camera.
            elif sensor_record['modality'] == 'camera' and not vis_indicators[-1]:
                continue
            # If we're not returning indicators, add the box since it's visible.
            else:
                box_list.append(box)

        if return_vis_indicators:
            return data_path, box_list, cam_intrinsic, vis_indicators
        else:
    ...
    def get_box_in_ego_frame(self, sample_annotation_token):
        box = self.get_box(sample_annotation_token)
        ann_record = self.get('sample_annotation', sample_annotation_token)
        sample_record = self.get('sample', ann_record['sample_token'])
        sample_data_record = self.get('sample_data', sample_record['data']['CAM_FRONT'])
...
class NuScenesExplorer:
    ...
    def get_sensors_transform(self, sample_record, ref_chan='LIDAR_TOP'):
        # Get reference pose and timestamp
        ref_sd_token = sample_record['data'][ref_chan]
        ...
        # Homogeneous transform from car frame to reference frame
        car_from_ref = transform_matrix(ref_cs_rec['translation'], Quaternion(ref_cs_rec['rotation']), inverse=True)
        return car_from_ref
    ...
    def render_sample_data(self,
        ...
        if sensor_modality == 'lidar':
            ...
            _, boxes, _ = self.nusc.get_sample_data(sample_data_token, box_vis_level=box_vis_level, selected_anntokens=selected_anntokens)
            ...
            _, boxes, _ = self.nusc.get_sample_data(lidar_token, box_vis_level=box_vis_level, selected_anntokens=selected_anntokens)
            ...
            # Load boxes and image. Use NONE visibility to get indicators of visibility, insetad.
            data_path, boxes, camera_intrinsic, vis_indicators = self.nusc.get_sample_data(
                sample_data_token,
                box_vis_level=BoxVisibility.ANY, 
                selected_anntokens=selected_anntokens,
                return_vis_indicators=True)
            ...
            if with_anns:
                for idx, box in enumerate(boxes):
                    ...
                    if colors is not None:
                        ...
                        c = np.array(self.get_color(box.name)) / 255.0
                    box.render(ax, view=camera_intrinsic, normalize=True, colors=(c, c, c), alpha=alpha)
    ...
    def get_annotations_with_attribute_names(self, sample, attribute_names=['vehicle.moving', 'vehicle.stopped']):
...
def interpolate_boxes_to_times(boxes, box_timestamps_relative, relative_times_interp):
    n_boxes = len(boxes)
    assert(n_boxes > 1)
...
def traverse_linked_list(nusc, obj, tablekey='sample', direction='prev', inclusive=False):
    ...
    :param tablekey: name of the object's sequence
